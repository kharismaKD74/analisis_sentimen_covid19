...
def LabelNB(df_label, df_unlabel):
    ...
    df_concat = pd.concat([df, df_predict])
    df_concat.rename(columns = {'sentimen':'sentimen', 'text':'text','clean_tweets1':'hasil_cleaning','clean_tweets2':'hasil_stop_word','clean_tweets3':'hasil_lemmatize'}, inplace=True)
...
def ModelLSTM():
    ...
    df_concat = pd.concat([X_train_pred_merge, X_val_pred_merge])
    df_concat2 = pd.concat([df_concat, X_test_pred_merge])

    df_concat2.to_csv("data hasil lstm.csv",index=False)
    ...
    create_wordcloud(df_concat2.loc[df_concat2['predicted']=='negative'], df_concat2['tweet'],"negative")
    create_wordcloud(df_concat2.loc[df_concat2['predicted']=='positive'], df_concat2['tweet'],"positive")
    create_wordcloud(df_concat2.loc[df_concat2['predicted']=='neutral'], df_concat2['tweet'],"neutral")

    return render_template('show_data_lstm.html', data=df_concat2.to_dict(orient='records'))
